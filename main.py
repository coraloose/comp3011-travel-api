...
from models import Place, Trip, TripPlace
from schemas import (
...
def delete_place(place_id: int, db: Session = Depends(get_db)):
    ...
    # 同时删除 trip_places 里引用这个 place 的记录
...
def list_trip_places(trip_id: int, db: Session = Depends(get_db)):
    ...
    # 让未分配 day 的排在后面，再按 day、planned_order 排序
    return (
        db.query(TripPlace)
...
def delete_trip_place(trip_id: int, trip_place_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(item)
    db.commit()
    return Nonefrom fastapi import FastAPI, Depends, HTTPException, status
from sqlalchemy.orm import Session
...
def delete_place(place_id: int, db: Session = Depends(get_db)):
    ...
    # 同时删除 trip_places 里引用这个 place 的记录（简单处理，避免外键问题）
    db.query(TripPlace).filter(TripPlace.place_id == place_id).delete()
